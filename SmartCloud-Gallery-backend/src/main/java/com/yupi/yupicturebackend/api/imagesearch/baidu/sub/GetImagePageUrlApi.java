    ...
    public static String getImagePageUrl(String imageUrl) {
        ...
        try {
            ...
            if (StrUtil.isBlank(rawUrl)) {
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "未返回有效的结果地址rawUrl");
            }
            String searchResultUrl = URLUtil.decode(rawUrl, StandardCharsets.UTF_8);
            // 如果 URL 为空
            if (StrUtil.isBlank(searchResultUrl)) {
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "未返回有效的结果地址searchResultUrl");
            }
            return searchResultUrl;
        } catch (Exception e) {
            log.error("接口调用失败，响应状态码：{}, 响应内容：{}", httpResponse.getStatus(), body);
